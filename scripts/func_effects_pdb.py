...
func_effects = snakemake.input.func_effects_csv
...
func_effects = pd.read_csv(func_effects).query('times_seen>2').copy()
...
dfs = []
for chain in chains:
    chain_df = func_effects[['site', 'effect']].copy()
    chain_df = chain_df.groupby(['site']).sum().reset_index()
